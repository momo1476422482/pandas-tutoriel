...
class analyse_csv:
    ...
    def impute_missing_values(self) -> None:
        if self.df["Experience"].isnull().values.any():
            ...
            list_metier = list(set(self.df['Metier'].to_numpy()))
            print(list_metier)

            for metier in list_metier[1:]:
                mean_value = self.df.query(f"Metier=='{metier}'")["Experience"].mean(
                    skipna=True
                )
                self.df.loc[
                    (self.df["Metier"] == metier), "Experience"
                ] = self.df.loc[
                    (self.df["Metier"] == metier), "Experience"
                ].fillna(
                    mean_value
    ...
    def aggregate_features(df: pd.DataFrame) -> np.ndarray:
        return np.c_[
            np.vstack(df["Technologies"]),
            df["Experience"].to_numpy().reshape(-1, 1),
            df["Diplome"].to_numpy().reshape(-1, 1),
        ]
    ...
    def predict_major(self) -> None:
        ...
        X_train, X_test, y_train, y_test = train_test_split(
            features_not_nan, df_without_na["Metier_transforme"], test_size=0.25
        )
        # RandomForest
        ...
        self.df.loc[(self.df["Metier"].isna()), "Metier"] = rf.predict(
            features_nan
        ).reshape(-1, 1)
        ...
        # GradientBoost
        gbf = GradientBoostingClassifier(n_estimators=100,learning_rate=1.0,max_depth=3, random_state=0)
        gbf.fit(X_train, y_train)
        accuracy = gbf.score(X_test, y_test)
        print(f"Mean accuracy score: {accuracy:.3}")
    ...
    def __call__(self, path_result):
        ...
        self.transform_feature(ac.get_most_used_technology(40))
        self.run_cluster()
        self.predict_major()
